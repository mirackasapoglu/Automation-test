...
test('Checkout formunda adres alanları var', async ({ page }) => {
  ...
  const labelExists = await checkoutPage.addressLabel.isVisible().catch(() => false);
  const textboxExists = await page.getByRole('textbox').first().isVisible().catch(() => false);
  const comboExists = await page.getByRole('combobox').first().isVisible().catch(() => false);
...
test('Ödeme yöntemi seçeneği var', async ({ page }) => {
  ...
  const headingExists = await checkoutPage.paymentHeading.isVisible().catch(() => false);
  const listExists = await page.locator('li').filter({ hasText: /kredi|banka|havale|eft|kapıda/i }).first().isVisible().catch(() => false);
  const radioCount = await page.locator('input[type="radio"]').count();
  const selectExists = await page.locator('select[name*="payment" i]').isVisible().catch(() => false);
...
test('Sipariş özeti görünüyor', async ({ page }) => {
  ...
  const summary = page.locator('[class*="summary" i], [class*="order" i][class*="review" i], [class*="review" i]').first();
