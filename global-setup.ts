...
async function globalSetup(_config: FullConfig) {
  ...
  if (!email || !password) {
    throw new Error('TEST_EMAIL veya TEST_PASSWORD .env içinde yok');
  }

  // Mevcut session geçerliyse login'i atla
  if (fs.existsSync(authFile)) {
    const isValid = await checkSession(authFile);
    if (isValid) {
  ...
  page.on('response', resp => {
    if (resp.url().includes('api.nadirgold.work/customer/login') && resp.status() === 200) {
...
async function checkSession(authFile: string): Promise<boolean> {
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext({ storageState: authFile });
  const page = await context.newPage();

  try {
    await page.goto('https://www.nadirgold.work/hesabim/siparislerim', {
      waitUntil: 'domcontentloaded',
      timeout: 30_000,
    });
    const url = page.url();
